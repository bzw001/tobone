...
define(['ajax','template','articleTemplate','jquery','utile'],function(ToBone,template,article,$,utile){
    /*获得ajax对象*/
    var ajax=new ToBone();
    /*1、轮播图的设置*/
    (function(){
    ...
    function blinkText(){
        ...
        Slideshow.prototype._navigate = function(direction) {
            ...
            /*调用textFx隐藏方法，显示定义在slideshow元素的data-effect属性里的特效number*/
    ...
    (function(){
        ...
        randomA.addEventListener("click",function(){
            ...
            moveEffect(randomSpan[1],'x',function(){
                flag=true;
                article.moveEffect(randomSpan[0],'y');
                setTimeout(function(){
    ...
    (function(){
        ...
        for(var j=0;j<fontLis.length;j++){
            fontLis[j].addEventListener("click",function(e){
                ...
                utile.setArg(e.target,{artId:str});
            })
        }
    ...
    ajax.ajax({
        ...
        success:function(result){
            ...
            article.showArticle('home','./php/mysqlTest.php');
            console.log('loading');
            $('.loading').hide();
            /*点击文章标题会将文章的id存入数据库*/
            var art_title=document.querySelectorAll('.article > article > a:first-of-type > h2');
            article.saveIdByCookie(art_title);
    ...
    ajax.ajax({
        ...
        success:function(result){
            ...
            /*数据更新完成后才执行点击喜欢事件的绑定*/
            article.likeHeader();
            var titleArr=document.querySelectorAll('.hot > ul > li > a:first-of-type');
            //console.log(titleArr);
            article.saveIdByCookie(titleArr);
        }
    });
});
